...
async def test_login_001_no_username_provided(monkeypatch, client, test_db, seed_user):
    ...
    response_data = response.json()
    print(response_data)
    assert response_data == {'detail': [{'type': 'missing', 'loc': ['body', 'username'], 'msg': 'Field required', 'input': {'password': 'password'}, 'url': 'https://errors.pydantic.dev/2.6/v/missing'}]}
    assert seed_user.refresh_token is None
    
    # Verify that the user was not logged in
    user_response = await test_db.execute(select(User).filter_by(username=seed_user.username))
...
@pytest.mark.asyncio
async def test_login_002_anomalous_no_password_provided(monkeypatch, client, test_db, seed_user):
    """
    Test 002 - Anomalous
    ...
    response_data = response.json()
    print(response_data)
    assert response_data == {'detail': [{'type': 'missing', 'loc': ['body', 'password'], 'msg': 'Field required', 'input': {'username': 'test-user'}, 'url': 'https://errors.pydantic.dev/2.6/v/missing'}]}
    assert seed_user.refresh_token is None
...
@pytest.mark.asyncio
async def test_login_003_anomalous_nonexistent_user(monkeypatch, client, test_db, seed_user):
    """
    Test 003 - Anomalous
    ...
    """
    # Make the request
    response = client.post("/api/v1/users/login", json={"username": "test-user2", "password": "password"})
...
async def test_login_004_anomalous_incortect_password(monkeypatch, client, test_db, seed_user):
    ...
    """
    # Make the request
    response = client.post("/api/v1/users/login", json={"username": "test-user", "password": "password2"})
    assert response.status_code == 401
    response_data = response.json()
    assert response_data == {"detail": "The provided credentials were incorrect"}
    assert seed_user.refresh_token is None
