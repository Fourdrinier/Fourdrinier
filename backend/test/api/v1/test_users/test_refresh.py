...
@pytest.mark.asyncio
async def test_refresh_000_nominal(monkeypatch, client, test_db, seed_user):
    """
    Test 000 - Nominal
...
async def test_refresh_001_anomalous_no_token(monkeypatch, client, test_db, seed_user):
    ...
    response_data = response.json()
    assert response_data["detail"] == [{'type': 'missing', 'loc': ['query', 'refresh_token'], 'msg': 'Field required', 'input': None, 'url': 'https://errors.pydantic.dev/2.6/v/missing'}]
...
@pytest.mark.asyncio
async def test_refresh_002_anomalous_no_username(monkeypatch, client, test_db, seed_user):
    """
    Test 002 - Anomalous
    ...
    assert response.status_code == 422
    response_data = response.json()
    assert response_data["detail"] == [{'type': 'missing', 'loc': ['query', 'client_id'], 'msg': 'Field required', 'input': None, 'url': 'https://errors.pydantic.dev/2.6/v/missing'}]
...
@pytest.mark.asyncio
async def test_refresh_003_anomalous_invalid_username(monkeypatch, client, test_db, seed_user):
    """
    Test 003 - Anomalous
...
@pytest.mark.asyncio
async def test_refresh_004_anomalous_invalid_token(monkeypatch, client, test_db, seed_user):
    """
    Test 004 - Anomalous
...
@pytest.mark.asyncio
async def test_refresh_005_anomalous_invalid_username_and_token(monkeypatch, client, test_db, seed_user):
    """
    Test 005 - Anomalous
